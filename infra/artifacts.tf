...
data "archive_file" "software_subagent_runtime" {
  type        = "zip"
  source_dir  = "${path.module}/runtime/subagent_software_builder"
  output_path = "${path.module}/software-subagent-runtime.zip"
  excludes    = ["**/__pycache__/**", "**/*.pyc"]
}

resource "aws_s3_object" "software_subagent_runtime" {
  bucket                 = aws_s3_bucket.agent_workspace.id
  key                    = "system/runtime/subagent-software-builder/${data.archive_file.software_subagent_runtime.output_sha256}/runtime.zip"
  source                 = data.archive_file.software_subagent_runtime.output_path
  source_hash            = data.archive_file.software_subagent_runtime.output_base64sha256
  content_type           = "application/zip"
  server_side_encryption = "AES256"
}
