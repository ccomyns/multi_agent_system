...
class SoftwareOrchestratorRunnerTests(unittest.TestCase):
    ...
    def test_codex_starts_at_repository_root_with_scoped_subagent_configuration(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("[features]\nmulti_agent = false", config)
            self.assertIn("mcp__software_agents__spawn_agent(task)", config)
            self.assertIn("mcp__software_agents__wait_on_any(agent_ids, timeout_seconds)", config)
