...
"""
    ...
    def run_codex(self, task: str) -> None:
        ...
        developer_instructions = (
            ...
            "Delegate data gathering exclusively through the software_agents MCP server: "
            "use mcp__software_agents__spawn_agent(task) to launch workers and "
            "you must use mcp__software_agents__wait_on_any(agent_ids, timeout_seconds) to collect results. "
            "If tools are deferred, discover these tools by their server and tool names. "
            "This server supports up to 12 concurrent subagents, each with a 30-minute total deadline. "
            "Each subagent runs on its own EC2 instance and uploads its work to a dedicated "
            "S3 folder under the assigned project prefix, <project_name>/<subagent_id>/. "
            "Assign each subagent exactly one website to webscrape; include the website URL, "
            "the data to gather, and all necessary task context in its assignment. "
            "Codex's built-in local subagent tools are disabled. Do not launch local Codex "
            "processes or use another delegation mechanism as a substitute. If the software_agents "
            "MCP tools are unavailable or fail, report the problem instead of bypassing the server. "
            "They have no GitHub, RDS or Vercel access; each can read and write only its "
            "assigned S3 folder and must upload gathered data as JSON files plus description.md. Include all needed context "
            ...
            "Inspect every new feature you add using Playwright with Chromium, exercise its "
            "user interactions, and fix any issues before publishing. "
            "Use the vercel_publisher MCP server's publish_site tool to publish the website "
            "you build and newer versions of that website as you make improvements, "
            "unless the user explicitly asks you not to publish. Publish only after the "
            "relevant tests and browser checks pass and every intended change is "
            ...
            + (
                ...
                "Implement S3 reads in server-side code with @aws-sdk/client-s3 and "
                "awsCredentialsProvider from @vercel/oidc-aws-credentials-provider, passing "
                "roleArn: process.env.AWS_ROLE_ARN. On Vercel, explicitly use those OIDC "
                "credentials and process.env.AWS_REGION for the S3 client. For local builder "
                "checks, use the existing AWS credential chain instead. Never copy the builder's "
                "AWS keys, session tokens, or credential files into the app or Vercel. "
                "Serve files through server routes, or generate short-lived presigned GET URLs "
                "after checking access; browser fetches to S3 also require matching bucket CORS. "
                "The production role can read the entire global-memory bucket but cannot write "
                "or delete; S3_PREFIX is the application's assigned location, not an IAM boundary. "
